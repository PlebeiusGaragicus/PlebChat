...
def load_convo(runlog):
    ...
    with open(os.path.join(dir, runlog), "r") as f:
        ...
        st.session_state.thread.messages = [deserialize_messages(m) for m in messages]

        st.session_state.thread.session_start_time = file_contents["session_start_time"]
        st.session_state.thread.description = file_contents["description"]
...
def delete_this_chat():
    ...
    runlog_file = os.path.join(st.session_state.runlog_dir, get('construct').name, f'{st.session_state.thread.session_start_time}.txt')
...
def save_chat_history() -> bool:
    ...
    if st.session_state.thread.description == None:
        new_chat_first_save = True
        # desc = st.session_state.model.get_description()
        # desc = "a new chat appears!"
        # desc = get_description()
        # desc = " ".join(st.session_state.thread.messages[0].content.split(" ")[:6]) # n=6
        desc = st.session_state.thread.messages[0].content
        ...
        desc = st.session_state.thread.description


    # serialize the messages
    messages = [serialize_messages(m) for m in st.session_state.thread.messages]
    ...
    with open(runlog_file, "w") as f:
        json.dump(
            {
                "description": desc,
                "session_start_time": st.session_state.thread.session_start_time,
                "messages": messages
            },
            f,
            indent=4
