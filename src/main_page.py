...
def main_page(appstate: ChatAppVars):
    ...
    if prompt:
        settings_sidebar.empty()
        # disable 'echobot' checkbox
        # st.session_state.echobot.disabled = True

        # settings_sidebar = st.container()
        # st.session_state.incomplete_stream = ""
        with st.sidebar:
            # with vanishing_sidebar:
            st.button("Interrupt", on_click=interrupt, key="button_interrupt")
